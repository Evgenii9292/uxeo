...
export function ContinueDisabled({ fullWidth }: { fullWidth?: boolean }) {
  return (
    <div className={`bg-[#333d41] content-stretch flex h-[56px] items-center justify-center px-[25px] relative rounded-[20px] shadow-[0px_5px_0px_0px_#212226] shrink-0 ${fullWidth ? "w-full" : ""}`}>
      <p className="font-['Roboto_Condensed:Bold',sans-serif] font-bold leading-[22.955px] relative shrink-0 text-[#465256] text-[22px] whitespace-nowrap">Продолжить</p>
    </div>
  );
}
...
export function ContinueActive({ onClick, fullWidth }: { onClick: () => void; fullWidth?: boolean }) {
  return (
    ...
    >
      ...
      <p className="font-['Roboto_Condensed:Bold',sans-serif] font-bold leading-[22.955px] text-[#f4f5fc] text-[22px] whitespace-nowrap">Продолжить</p>
    </button>
  );
}
...
export function ContinueCorrect({ onClick, fullWidth }: { onClick: () => void; fullWidth?: boolean }) {
  return (
    <button
      onClick={() => { onClick(); playClickSound(); }}
      className={`group relative flex h-[56px] items-center justify-center px-[25.831px] py-[0.831px] rounded-[20px] shrink-0 bg-[#00bb3c] cursor-pointer select-none outline-none transition-transform duration-75 hover:translate-y-[3px] active:translate-y-[5px] ${fullWidth ? "w-full" : ""}`}
    >
      <div aria-hidden="true" className="absolute border-[#00b93c] border-[0.831px] border-solid inset-0 pointer-events-none rounded-[20px] transition-[box-shadow] duration-75 shadow-[0px_5px_0px_0px_#668562] group-hover:shadow-[0px_2px_0px_0px_#668562] group-active:shadow-none" />
      <p className="font-['Roboto_Condensed:Bold',sans-serif] font-bold leading-[22.955px] text-[#f4f5fc] text-[22px] whitespace-nowrap">Продолжить</p>
    </button>
  );
}